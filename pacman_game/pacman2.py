...
def loadAgent(pacman, nographics):
    """
    優先從 ghost_bt 載入（含經典→BT 的別名映射），
    回退掃描 *Agents.py 時排除 ghostAgents.py。
    ※ 一旦確定來源，就印出「檔案路徑與類別名稱」以便確認。
    """
    import os, importlib

    name = (pacman or "").strip()

    # 經典鬼名 → BT 類別
    bt_alias = {
        'RandomGhost': 'BTRandomGhost',
        'DirectionalGhost': 'BTDirectionalGhost',
        'ChasingGhost': 'BTChasingGhost',
        'ImperfectGhost': 'BTImperfectGhost',
    }

    # 小工具：印出來源
    def _announce(module, cls_name):
        src = getattr(module, "__file__", "<built-in>")
        print(f"[Ghost Source] {cls_name}  <-  {src}")
        return getattr(module, cls_name)

    # 1) 先嘗試從 ghost_bt 載入（直接名或別名）
    try:
        ghost_bt = importlib.import_module("ghost_bt")
        if hasattr(ghost_bt, name):
            return _announce(ghost_bt, name)
        if name in bt_alias and hasattr(ghost_bt, bt_alias[name]):
            return _announce(ghost_bt, bt_alias[name])
    except ImportError:
        pass

    # 2) 回退掃描 *Agents.py，但排除 ghostAgents.py
    pythonPathStr = os.path.expandvars("$PYTHONPATH")
    if ';' in pythonPathStr:
        pythonPathDirs = pythonPathStr.split(';')
    else:
        pythonPathDirs = [p for p in pythonPathStr.split(':') if p]
    pythonPathDirs.append('.')

    for moduleDir in pythonPathDirs:
        if not os.path.isdir(moduleDir):
            continue
        moduleNames = [f for f in os.listdir(moduleDir)
                       if f.endswith('gents.py') and f != 'ghostAgents.py']
        for modulename in moduleNames:
            modname = modulename[:-3]
            try:
            ...
            if hasattr(module, name):
                ...
                return _announce(module, name)

    raise Exception('The agent ' + name + ' is not specified in any *Agents.py or ghost_bt.py.')

def replayGame(layout, actions, display):
    import PacmanAgents, importlib
    ghost_bt = importlib.import_module("ghost_bt")

    # 印出重播時鬼的來源
    print(f"[Ghost Source] BTRandomGhost  <-  {getattr(ghost_bt, '__file__', '<built-in>')}")

    rules = ClassicGameRules()
    agents = [PacmanAgents.GreedyAgent()] + [
        ghost_bt.BTRandomGhost(i + 1) for i in range(layout.getNumGhosts())
    ]
